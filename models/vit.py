...
class SupConVit(nn.Module):
    ...
    def __init__(self, name='vit_small_patch16_224', head='mlp', feat_dim=128):
        super(SupConVit, self).__init__()
        # model_fun = model_dict[name]
        vit = model_dict[name](pretrained=True)
        # vit = models.vit_b_16(pretrained=True)

        self.encoder = vit
        if head == 'linear':
            self.head = nn.Linear(vit.embed_dim, feat_dim)
        elif head == 'mlp':
            self.head = nn.Sequential(
                nn.Linear(vit.embed_dim, vit.embed_dim),
                nn.ReLU(inplace=True),
                nn.Linear(vit.embed_dim, feat_dim)
            )
        else:
...
model_dict = {
    ...
    'vit_small_patch16_224': timm_vit.vit_small_patch16_224,
