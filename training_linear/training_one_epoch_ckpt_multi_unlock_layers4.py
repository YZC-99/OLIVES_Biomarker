...
def train_OCT_multilabel_unlock_layer4(train_loader, model, classifier, criterion, optimizer, epoch, opt):
    """one epoch training"""
    # model.eval()
    classifier.train()
...
def main_multilabel():
    ...
    for i in range(0,1):
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            # train for one epoch
            time1 = time.time()
            loss, acc = train_OCT_multilabel_unlock_layer4(train_loader, model, classifier, criterion,
                              optimizer, epoch, opt)
            time2 = time.time()
